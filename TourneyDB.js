...
class TourneyDB {
    ...
    static initialize() {
        this.db.serialize(() => {
            this.db.run('CREATE TABLE Tourneys (id INTEGER PRIMARY KEY, fname TEXT NOT NULL, lname TEXT NOT NULL, email TEXT NOT NULL);');
        ...
        return new Promise((resolve, reject) => {
            this.db.all(`SELECT * from Tourneys where (id == ${id})`, (err, rows) => {
                if (rows.length >= 1) {
                    ...
                    reject(`Tourneys with Id ${id} not found`);
                }
            });
        });
    }
